...
select player,count(distinct matchid) as total_match_played ,
count(case when batter_cnt =1 then 1 else null end) as batted,
count(case when baller_cnt =1 then 1 else null end) as balled


from (
select batter as player, matchid ,
1 as batter_cnt ,0 as baller_cnt
from cricket_match
union 
select baller as player, matchid,
0 as batter_cnt ,1 as baller_cnt from cricket_match ) a
group by 1
